...
from experiments.entrypoints._abstract_entrypoint import run_experiment, _main, run_experiment_ltlfond2fond
...
def _do_job(workdir: Path, timeout: float):
    ...
    for heuristic in Heuristic:
        if (heuristic, ) in combination_already_failed:
            print(f"Skipping configuration {(heuristic, )} for {0=}")
            logging.info(f"Skipping configuration {(heuristic, )} for {0=}")
            continue
        result = run_experiment_ltlfond2fond(workdir, timeout,
                                f"electric_motor_det_ltlfond2fond_{heuristic.value}",
                                build_services, build_goal, heuristic)
        # don't skip
