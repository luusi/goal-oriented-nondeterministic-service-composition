...
CHIP_PRODUCTION_NONDET_LTLFOND2FOND = "chip_production_nondet_unsolvable"


def get_chip_production_nondet_unsolvable_ltlfond2fond_dataframe(input_dir: Path) -> pd.DataFrame:
    df = pd.DataFrame(columns=["label", "length", "heuristic", "TT", "PT", "NE", "PS"])
    prefix = "chip_production_nondet_unsolvable"
    for length in range(1, 13):
        for heuristic in heuristic_list():
            dirname = f"{prefix}_len_{length}_{heuristic.value}"
            result_dirpath = input_dir / dirname
            row_prefix = [CHIP_PRODUCTION_NONDET_LTLFOND2FOND, length, heuristic.value]
...
def main():
    ...
    df = get_chip_production_nondet_unsolvable_ltlfond2fond_dataframe(input_dir)
    ...
    plt.ylim(0, 1000.0)
    plt.xticks(ticks=x_axis.tolist(), labels=ExpType.CHIP_PRODUCTION_NONDET_UNSOLVABLE.labels(), rotation=15)
    plt.title("Chip production (nondeterministic, unsolvable)")

    plt.savefig(output_dir / f"{ExpType.CHIP_PRODUCTION_NONDET_UNSOLVABLE.value}.pdf")
