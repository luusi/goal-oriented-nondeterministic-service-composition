...
CHIP_PRODUCTION_NONDET_TB = "chip_production_nondet_tb"
CHIP_PRODUCTION_NONDET_LTLFOND2FOND = "chip_production_nondet_ltlfond2fond"


def get_chip_production_nondet_ltlfond2fond_dataframe(input_dir: Path) -> pd.DataFrame:
    df = pd.DataFrame(columns=["label", "length", "heuristic", "TT", "PT", "NE", "PS"])
    prefix = "chip_production_nondet"
...
def main():
    ...
    df = get_chip_production_nondet_ltlfond2fond_dataframe(input_dir)
    ...
    plt.ylim(0, 1000.0)
    plt.xticks(ticks=x_axis.tolist(), labels=ExpType.CHIP_PRODUCTION_NONDET.labels(), rotation=15)
    plt.title("Chip production (nondeterministic)")

    plt.savefig(output_dir / f"{ExpType.CHIP_PRODUCTION_NONDET.value}.pdf")
