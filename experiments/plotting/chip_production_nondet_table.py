...
def _populate_table(input_dir, table: Tabular, prefix: str, title: str, unsolvable: bool=False):
    exptypes = [f"{prefix}_len_{i}" for i in range(1, len(ALL_SYMBOLS_CHIP_PRODUCTION) + 1)]
    labels = ExpType.CHIP_PRODUCTION_NONDET.labels() if not unsolvable else ExpType.CHIP_PRODUCTION_NONDET_UNSOLVABLE.labels()
    ...
    for length in range(1, 13):
        ...
        for heuristic in Heuristic:
            resultdirname = f"{prefix}_len_{length}_{heuristic.value}"
...
def main():
    ...
    with doc.create(Tabular(table_config)) as table:
        _populate_table(input_dir, table,  "chip_production_nondet", r"\textbf{Chip Production scenario (nondeterministic)}")
    (output_dir / f"table_chip_production_nondet.tex").write_text(table.dumps())
    doc.generate_pdf(str(output_dir / f'table_chip_production_nondet.pdf'), clean_tex=False)

    doc = _get_document()
    with doc.create(Tabular(table_config)) as table:
        _populate_table(input_dir, table,  "chip_production_nondet_unsolvable", r"\textbf{Chip Production scenario (nondeterministic, unsolvable)}", unsolvable=True)
    (output_dir / f"table_chip_production_nondet_unsolvable.tex").write_text(table.dumps())
    doc.generate_pdf(str(output_dir / f'table_chip_production_nondet_unsolvable.pdf'), clean_tex=False)
