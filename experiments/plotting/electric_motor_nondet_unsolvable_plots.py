...
ELECTRIC_MOTOR_NONDET_UNSOLVABLE = "electric_motor_nondet_unsolvable"


def get_electric_motor_nondet_unsolvable_ltlfond2fond_dataframe(input_dir: Path) -> pd.DataFrame:
    df = pd.DataFrame(columns=["label", "heuristic", "TT", "PT", "NE", "PS"])
    prefix = ELECTRIC_MOTOR_NONDET_UNSOLVABLE
    for heuristic in heuristic_list():
        dirname = f"{prefix}_{heuristic.value}"
        result_dirpath = input_dir / dirname
        row_prefix = [ELECTRIC_MOTOR_NONDET_UNSOLVABLE, heuristic.value]
...
def main():
    ...
    df = get_electric_motor_nondet_unsolvable_ltlfond2fond_dataframe(input_dir)
    ...
    plt.ylim(0, 1000.0)
    plt.xticks(ticks=x_axis.tolist(), labels=ExpType.ELECTRIC_MOTOR_NONDET_UNSOLVABLE.labels(), rotation=15)
    plt.title("Electric Motor (nondeterministic, unsolvable)")

    plt.savefig(output_dir / f"{ExpType.ELECTRIC_MOTOR_NONDET_UNSOLVABLE.value}.pdf")
